 ...
 * Groups consecutive questions by their shared question_context and shows it
 * as a section header. Renders each question as a text input, radio, or select.
 */
export default function FallbackRenderer({
  ...
  // Group consecutive questions by their shared question_context.
  type CtxGroup = { context: string | null; questions: BackendQuestion[] };
  const ctxGroups = React.useMemo((): CtxGroup[] => {
    const result: CtxGroup[] = [];
    for (const q of group.questions) {
      const ctx = q.question_context ?? null;
      if (result.length === 0 || result[result.length - 1].context !== ctx) {
        result.push({ context: ctx, questions: [q] });
      } else {
        result[result.length - 1].questions.push(q);
      }
    }
    return result;
  }, [group.questions]);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-10">
        {ctxGroups.map((ctxGroup, gi) => (
          <div key={gi} className="space-y-4">
            {ctxGroup.context && (
              <p className="text-base font-bold text-ink leading-relaxed border-b border-rule pb-3">
                {ctxGroup.context}
              </p>
            )}
            <div className="space-y-4">
              {ctxGroup.questions.map((q) => {
                  ...
                  <div
                    ...
                    className={cn(
                      ...
                      flashQuestionNumber === q.question_number &&
                        "ielts-question-flash rounded-md bg-mint-soft p-3 -mx-3",
                    )}>
                    <div className="flex items-baseline gap-x-2 gap-y-2">
                    ...
                    {hasOptions && (isRadio || isCheckbox) && (
                      <div className="ml-10 space-y-2">
                        {q.options.map((opt) => (
                          <label
                            key={opt.id}
                            className="flex items-start gap-3 cursor-pointer hover:bg-paper-2 rounded-lg px-3 py-2 -mx-3">
                            <input
